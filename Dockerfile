...
RUN curl -L -O https://s3.amazonaws.com/zcsdocker/soapvalidator-20170912.tar.gz
RUN curl -L -O http://downloads.sourceforge.net/project/staf/staf/V3.4.26/STAF3426-setup-linux-amd64-NoJVM.bin
RUN mkdir -p /opt/qa && \
    tar xzvf /tmp/soapvalidator-20170912.tar.gz -C /opt/qa/ && \
    ...
    rm /tmp/soapvalidator-20170912.tar.gz && \
    echo /usr/local/staf/lib > /etc/ld.so.conf.d/zzz-staf.conf && \
    ldconfig

COPY ./init /opt/qa/init
